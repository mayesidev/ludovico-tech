...
const inputPath = scriptArgs[0] ?? "data/Movie List (Responses) - Form Responses 1.csv";
const outputPath = scriptArgs[1] ?? "data/generated-import.sql";
const rows = parse(readFileSync(resolve(inputPath)), { skip_empty_lines: true, relax_column_count: true }) as Row[];
rows.shift();
...
const parseImdbId = (value: string) => value.match(/\/title\/(tt\d+)/i)?.[1] ?? null;
...
for (const [rowIndex, row] of rows.entries()) {
  ...
  if (duplicateKeys.has(duplicateKey)) { skipped += 1; continue; }
  duplicateKeys.add(duplicateKey);

  const franchiseName = row[4]?.trim() || null;
  const franchiseKey = franchiseName ? normalize(franchiseName) : null;
  const franchiseId = franchiseKey ? franchiseIds.get(franchiseKey) ?? randomUUID() : null;
  if (franchiseKey && franchiseName && !franchiseIds.has(franchiseKey)) {
    franchiseIds.set(franchiseKey, franchiseId!);
    statements.push(`INSERT OR IGNORE INTO franchises (id, name, created_at, updated_at) VALUES (${sql(franchiseId)}, ${sql(franchiseName)}, ${sql(createdAt)}, ${sql(createdAt)})`);
  }
  const position = franchiseKey ? (franchisePositions.get(franchiseKey) ?? 0) + 1 : null;
  if (franchiseKey && position !== null) franchisePositions.set(franchiseKey, position);
  const rating = parseRating(row[6] ?? "");
  const movieId = randomUUID();
  const sourceAddedAt = parseDate(row[0] ?? "");

  statements.push(`INSERT OR IGNORE INTO movies (id, title, title_normalized, added_at, source_added_at, source_row, updated_at, release_date, imdb_id, franchise_id, prior_viewed, rating_score, rating_phrase, watched_at) VALUES (${sql(movieId)}, ${sql(title)}, ${sql(normalize(title))}, ${sql(sourceAddedAt ?? createdAt)}, ${sql(sourceAddedAt)}, ${rowIndex + 2}, ${sql(createdAt)}, NULL, ${sql(imdbId)}, ${sql(franchiseId)}, ${row[2]?.trim().toLowerCase() === "yes" ? 1 : 0}, ${sql(rating.score)}, ${sql(rating.phrase)}, NULL)`);
  if (franchiseId && position !== null && franchiseName) {
    statements.push(`INSERT OR IGNORE INTO franchise_movies (franchise_id, movie_id, position) VALUES (${sql(franchiseId)}, ${sql(movieId)}, ${position})`);
  }
  imported += 1;
}
...
for (let start = 0; start < bodyStatements.length; start += chunkSize) {
  ...
  writeFileSync(resolve(chunkPath), `BEGIN TRANSACTION;\n${chunk.join(";\n")};\nCOMMIT;\n`);
  chunkCount += 1;
}
console.log(`Prepared ${imported} movies (${skipped} skipped) in ${chunkCount} local SQL chunks`);
