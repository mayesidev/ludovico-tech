...
export const getMovie = async (env: AppEnv["Bindings"], id: string) =>
  env.DB.prepare(`${movieSelect} WHERE movies.id = ?`).bind(id).first<MovieRow>();
...
export const getRemainingFranchiseMovies = async (env: AppEnv["Bindings"], franchiseId: string) => {
