...
test("adds a movie, rolls it, and records its final rating", async ({ page }) => {
  await page.goto("/");

  await expect(page.getByRole("heading", { name: "What’s on the marquee?" })).toBeVisible();
  ...
  await page.getByPlaceholder("Give it a goofy phrase…").fill("A browser-tested classic");
  await page.getByRole("button", { name: "Rate it", exact: true }).click();

  await expect(page.getByText(/Unknown date · Watched/)).toBeVisible();
  await expect(page.getByText(/A browser-tested classic/).first()).toBeVisible();
});
